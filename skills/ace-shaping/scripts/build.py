...
def _get_instructions(operation: str) -> None:
    """Load engine, get ace-shaping skill, print display_content(operation)."""
    if sys.stdout.encoding and sys.stdout.encoding.lower() not in ("utf-8", "utf8"):
        sys.stdout.reconfigure(encoding="utf-8")
    engine_root = _skill_dir
    config_path = _skill_dir / "conf" / "ace-config.json"
    project_config = _skill_dir.parent.parent / "conf" / "ace-config.json"
    ...
    skill = engine.get_skill("ace-shaping") or (engine.skills[0] if engine.skills else None)
    if not skill:
        print("ERROR: No ace-shaping skill loaded.", file=sys.stderr)
        sys.exit(1)
